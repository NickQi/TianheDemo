    ...
    loadPage.prototype = {
        ...
        */
        ...
        searchAction: function (currentPage, isPadding) {
            ...
            jQuery.ajax({
                url: this.getAction().GetUserList,
                dataType: "json",
                type: "POST",
                data: { "Inputs": data },
                success: function (json) {
                    ...
                    if (json.ResultInfo.Success) {
                        new loadPage().showUserList(json);
                    } else {
        ...
        showUserList: function (json) {
		    ...
		    html = "";
            if (json.UserList != null && json.UserList.length > 0) {

                html = template.render("userConfigItem", json);
                $list.html(html);
            } else {
        ...
        */
        getAction: function () {
            return {
                GetUserList: "action.ashx?action=GetUserList",
                AddUser: "action.ashx?action=AddUser",
                EditUser: "action.ashx?action=EditUser",
                DeleteUser: "action.ashx?action=DeleteUser",
                GetUserInfo: "action.ashx?action=GetUserInfo",
                GetAllUserGroups: "action.ashx?action=GetAllUserGroups"
            };
        }
    };
