...
function _4(_5){
var _6=$.data(_5,"tree").options;//console.log($.data(_5,"tree"));
...
//add by pl 2014-3-18 for select all children
if(tt.hasClass("select-all")){
var parentnode=_c(_5,_9);
var flag = 1;
var selectednodes;
for(var i in parentnode.children){
	...
	$.each($treeVisible,function(i,item){
		if($(item).is(":visible")){
			selectednodes=$(item).find(".tree-node-selected");
		}
	});	
	if(selectednodes && selectednodes.length>=_6.maxlen.num){
		//alert(_6.maxlen.detail+"最多选择"+_6.maxlen.num+"个对象！");
		$tiansu.common.info('show',{
							...
							content: _6.maxlen.detail + "最多选择" + _6.maxlen.num + "个对象！",
							timeout:2000
						});
		flag = 0;
		return;
	...
	if(flag == 1){
		tt.addClass("select-none").removeClass("select-all").text("all-");
	}
	var item=parentnode.children[i];
	$("#"+item.domId).addClass("tree-node-selected");
}
return;	
}
if(tt.hasClass("select-none")){	
tt.removeClass("select-none").addClass("select-all").text("all+");	
var parentnode=_c(_5,_9);
for(var i in parentnode.children){
	var item=parentnode.children[i];
	$("#"+item.domId).removeClass("tree-node-selected");
}
return;
}
if(tt.hasClass("tree-hit")){
...
//modify by pl for return array of selected nodes 2014-3-17
function _bf(_c0){
var _c1=$(_c0).find("div.tree-node-selected");
var _pl=[];
for( var i=0;i<_c1.length;i++){
_pl.push(_c1.length?_c(_c0,_c1[i]):null);
}
console.log(_pl);
return _pl;
};
...
//modify by pl for single or multiple select
if(_d9.single){
	$(_d7).find("div.tree-node-selected").removeClass("tree-node-selected");
	$(_d8).addClass("tree-node-selected");
}else{
	if($(_d8).hasClass("tree-node-selected")){
		$(_d8).removeClass("tree-node-selected");
	}else{
		$(_d8).addClass("tree-node-selected");
	}
}
...
var t=$(_10f);
//modify by pl add single and selectchildren 2014-3-17
return $.extend({},$.parser.parseOptions(_10f,["url","method",{checkbox:"boolean",single:"boolean",maxlen:"object",selectchildren:"boolean",data:"object",cascadeCheck:"boolean",onlyLeafCheck:"boolean"},{animate:"boolean",lines:"boolean",dnd:"boolean"}]));
};
...
//remove classname tree-folder by pl 2014-3-18
...
//add by pl for select all childern 2014-3-17
if(opts.selectchildren){
if(item.children.length && opts.single == false){
cc.push("<span class='select-all pl5 pr5 ml5'>all+</span>");
}
}
cc.push("</div>");
if(item.children&&item.children.length){
...
$.ajax({type:opts.method,url:opts.url,data:opts.data,dataType:"json",async: false,success:function(data){
_11b(data);
},error:function(){
