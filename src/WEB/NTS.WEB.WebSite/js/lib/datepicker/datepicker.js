...
(function($, undefined) {
	...
	Datepicker.prototype = {
		...
		getClassNames: function(date, flag) {
			...
			if (date.getUTCFullYear() < year || (date.getUTCFullYear() == year && date.getUTCMonth() < month)) {
				cls.push('old');
			} else if (date.getUTCFullYear() == year && date.getUTCMonth() > month || date.getUTCFullYear() > year && date.getUTCMonth() - month > -12) {				  
				cls.push('new');
			}
		...
		fill: function() {
			...
			this.picker.find('.leftCalendar .datepicker-days thead th.datepicker-switch')
				.html('<b>' + year + '</b>' + dates[this.o.language].year[0] + dates[this.o.language].months[month]);
			this.picker.find('.leftCalendar .month-num').text(month + 1);
			//改成单日历 pl 2014-3-19 20:53:15
			//this.picker.find('.leftCalendar thead th.next i').remove();
			//this.picker.find('.leftCalendar thead th.next').removeAttr('class').removeAttr('style');
			this.picker.find('.rightCalendar .datepicker-days thead th.datepicker-switch')
				.html('<b>' + rYear + '</b>' + dates[this.o.language].year[0] + dates[this.o.language].months[rMonth]);
			this.picker.find('.rightCalendar .month-num').text(rMonth + 1);			
			this.picker.find('.rightCalendar thead th.prev i').remove();
			this.picker.find('.rightCalendar thead th.prev').removeAttr('class').removeAttr('style');
			// show today text		
			var todayText = this.picker.find('.range-date .today');
			if (todayText.length === 0 && this.o.todayBtn !== false) {
				this.picker.find('.range-date')
					.prepend('<a class="today">' + dates[this.o.language].today + "</a>")
				//.toggle(this.o.todayBtn !== false);
			}
			var latestText = this.picker.find('.range-date .latest-date');
			if (latestText.length === 0 && typeof this.o.rangeDate === "function") {
				this.picker.find('.range-date')
			...
			while(prevMonth.valueOf() < nextMonth) {
				...
				if (prevMonth.getUTCMonth() + 1 - month === 1) {						
					if (prevMonth.getDay() === 0) {
						tclsName = "red";
					} else if (this.o.weekStart === 0 && prevMonth.getDay() > 5) {
						tclsName = "red";
					} else if (this.o.weekStart === 1 && prevMonth.getDay() > 5) {
						tclsName = "red";
					}
				}
			...
			var currentMonth = UTCDate(year, month, 28),
				day = DPGlobal.getDaysInMonth(currentMonth.getUTCFullYear(), currentMonth.getUTCMonth());
			currentMonth.setUTCDate(day);
			currentMonth.setUTCDate(day - (currentMonth.getUTCDay() - this.o.weekStart + 7) % 7);
			...
			while (currentMonth.valueOf() < rightNextMonth) {
				if (currentMonth.getUTCDay() == this.o.weekStart) {
					rhtml.push('<tr>');
					if(this.o.calendarWeeks){
						// ISO 8601: First week contains first thursday.
						// ISO also states week starts on Monday, but we can be more abstract here.
				...
				//rightClsName = $.unique(clsName);
				// edit by ghj 2013-06-29 20:55:58
				var tclsName = "";
				if (currentMonth.getUTCMonth() + 1 - prevMonth.getUTCMonth() === 1) {						
					if (currentMonth.getDay() === 0) {
						tclsName = "red";
					} else if (this.o.weekStart === 0 && currentMonth.getDay() > 5) {
						tclsName = "red";
					} else if (this.o.weekStart === 1 && currentMonth.getDay() > 5) {
						tclsName = "red";
					}
				}
				if(currentMonth.getUTCMonth() === prevMonth.getUTCMonth()) {
			...
			var months = this.picker.find('.leftCalendar .datepicker-months')
				.find('th:eq(1)')
				.text(year + dates[this.o.language].year[0])
				.end()
			...
			var rightMonths = this.picker.find('.rightCalendar .datepicker-months')
				.find('th:eq(1)')
				.text((year + 1) + dates[this.o.language].year[0])
				.end()
				.find('span').removeClass('active');
			...
			var yearCont = this.picker.find('.leftCalendar .datepicker-years')
				.find('th:eq(1)')
				.text(year + '-' + (year + 9) + dates[this.o.language].year[0])
				.end()
				.find('td');
			year -= 1;			
			var today = new Date(),
			...
			var rYearCont = this.picker.find('.rightCalendar .datepicker-years')
				.find('th:eq(1)')
				.text(rYear + '-' + (rYear + 9) + dates[this.o.language].year[0])
				.end()
				.find('td');
			rYear -= 1;
			for (var i = -1; i < 11; i++) {
				...
				switch (target[0].nodeName.toLowerCase()) {
					...
					case 'span':
						if (!target.is('.disabled')) {
							...
							if (target.is('.month')) {
								...
								if (len === 1) {
									year = this.viewDate.getUTCFullYear() + 1;
								} else {
									year = this.viewDate.getUTCFullYear();
								}
					...
					case 'td':
						if (target.is('.day') && !target.is('.disabled')) {
							...
							if (typeof(this.o.stepDate) === "object" && this.o.stepDate.type !== "") {
								var yearNum = 0,
									monthNum = 0;
								if (this.o.stepDate.type === "month") {
									yearNum = 0;
									monthNum = -1;
								} else if(this.o.stepDate.type === "quarter") {									
									yearNum = 0;
									monthNum = -3;
								} else if (this.o.stepDate.type === "year") {									
									yearNum = 1;
									monthNum = 0;
								}
								...
								if(UTCDate(date.getFullYear() + yearNum, date.getMonth()+1, 0, 0, 0, 0).getDate() == sDate.getDate() && monthNum == -1){
									eDate = UTCDate(date.getFullYear() + yearNum, date.getMonth(), 1, 0, 0, 0);
								}
								...
								var startDate = DPGlobal.formatDate(sDate, this.o.format, this.o.language),
									endDate = DPGlobal.formatDate(eDate, this.o.format, this.o.language);
									if(monthNum = 0){
										this.o.stepDate.callback.call(this, startDate, endDate);
									}else{
										this.o.stepDate.callback.call(this, endDate, startDate);
									}								
							} else {
							this._setDate(UTCDate(year, month, day));
							}
					...
					case 'a':
						switch (target[0].className) {
							case 'today':
								...
								break;
							case 'latestDay':
								var date = this.o.serverDate ? new Date(this.o.serverDate) : new Date();alert(this.o.serverDate);
								var sDate = UTCDate(date.getFullYear(), date.getMonth(), date.getDate(), 0, 0, 0),
									eDate = date;
								var startDate = DPGlobal.formatDate(sDate, this.o.format, this.o.language),
									endDate = DPGlobal.formatDate(eDate, this.o.format, this.o.language);

								if (typeof this.o.rangeDate === "function") {
									this.o.rangeDate.call(this, startDate, endDate);
								...
								break;
							case 'latestWeek':
								var date = this.o.serverDate ? new Date(this.o.serverDate) : new Date();alert(date);
								...
								if (typeof this.o.rangeDate === "function") {
									...
									month =sDate.getUTCMonth();
									day = sDate.getDate();
                                    //comment out by pl 2013-12-09
									//this._setDate(UTCDate(year, month, day));
								}
								break;
							case 'latestMonth':
								var date = this.o.serverDate ? new Date(this.o.serverDate) : new Date();
	...
	var dates = $.fn.datepicker.dates = {
		en: {
			...
			months: ["<b>1</b>月", "<b>2</b>月", "<b>3</b>月", "<b>4</b>月", "<b>5</b>月", "<b>6</b>月", "<b>7</b>月", "<b>8</b>月", "<b>9</b>月", "<b>10</b>月", "<b>11</b>月", "<b>12</b>月"],
